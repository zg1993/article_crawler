...
from crawler.utils import timestamp_to_str, get_time_now
...
async def search_result(session: aiohttp.ClientSession, search, now_str, delta=0, **kwargs):
    ...
    while count < 10:
        ...
        print('search_html: {}'.format(len(search_html)))
        ...
        print('sleep {}'.format(time.time() - start))
    filter_arr = []
    print(len(res))
    # pprint(res)
    for i in res:
        ...
        if timestamp_to_str(update_time, fmt='%Y-%m-%d') >= now_str:
            ...
            if weixin_link:
                ...
                print('error resolve sogou link failed')
                print(link_html )
            if update_time not in aid_keys:
                aid_keys.add(update_time)
                filter_arr.append(i)
            else:
                print(f'duplicate {update_time} title: {i["title"]}  link: {weixin_link}')
